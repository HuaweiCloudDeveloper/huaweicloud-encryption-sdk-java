...
public class CryptoHandlerTest {
    ...
    public void Should_ok_When_CryptoHandlerData() throws NoSuchAlgorithmException, NoSuchProviderException {
        ...
        List<byte[]> bytes = Utils.readMasterKey(Collections.singletonList("src/test/resources/128bit"));
        SecretKey secretKey = new SecretKeySpec(bytes.get(0), "SM4");
        dataKeyMaterials.setPlaintextDataKey(secretKey);
        CryptoResult<byte[]> encrypt = defaultCryptoHandler.encrypt(new EncryptRequest(Collections.emptyMap(), "hello world".getBytes(StandardCharsets.UTF_8)), dataKeyMaterials);
    ...
    public void Should_ok_When_CipherHandlerCipherData() {
        List<byte[]> bytes = Utils.readMasterKey(Collections.singletonList("src/test/resources/128bit"));
        SecretKey secretKey = new SecretKeySpec(bytes.get(0), "SM4");
        CipherHandler cipherHandler = new CipherHandler(CryptoAlgorithm.SM4_128_GCM_NOPADDING, secretKey, Cipher.ENCRYPT_MODE);
        ...
        Assert.assertEquals(new String(decryptData), new String(plaintText));
    }
    ...
    public void Should_ok_When_CipherHandlerProcessData() {
        List<byte[]> bytes = Utils.readMasterKey(Collections.singletonList("src/test/resources/128bit"));
        SecretKey secretKey = new SecretKeySpec(bytes.get(0), "SM4");
        FileEncryptHandler cipherHandler = new FileEncryptHandler(CryptoAlgorithm.SM4_128_GCM_NOPADDING, secretKey, Cipher.ENCRYPT_MODE);
        try (InputStream inputStreamEncrypt = new FileInputStream("src/test/resources/128bit");
        ...
        FileDecryptHandler fileDecryptHandler = new FileDecryptHandler(CryptoAlgorithm.SM4_128_GCM_NOPADDING, secretKey, Cipher.DECRYPT_MODE);
    ...
    public void Should_Exception_When_EncryptedFileChanged() throws IOException {
        ...
        SecretKey secretKey = new SecretKeySpec(bytes.get(0), "SM4");
        FileEncryptHandler cipherHandler = new FileEncryptHandler(CryptoAlgorithm.SM4_128_GCM_NOPADDING, secretKey, Cipher.ENCRYPT_MODE);
        try (InputStream inputStreamEncrypt = Files.newInputStream(Paths.get("src/test/resources/128bit"));
             FileOutputStream outputStreamEncrypt = new FileOutputStream("src/test/resources/128bit.encrypted")) {
            cipherHandler.processByte(inputStreamEncrypt, outputStreamEncrypt, Collections.emptyMap(), 4096);
        } catch (Exception e) {
            e.printStackTrace();

...
public class KmsDataKeyGenerate implements DataKeyGenerate {
    ...
    public void dataKeyGenerate(HuaweiConfig huaweiConfig, DataKeyMaterials dataKeyMaterials) {
        ...
        for (KMSConfig kmsConfig : kmsConfigList) {
            ...
            dataKeyMaterials.setPlaintextDataKey(new SecretKeySpec(plaintBytes, huaweiConfig.getCryptoAlgorithm().getKeySpec()));
            return;
        }
    }

    private CreateDatakeyResponse createDataKeyByKms(String regionStr, String keyId, String endPoint, ICredential auth, CryptoAlgorithm algorithm) {
        try {
            Region region;
            ...
            CreateDatakeyRequestBody body = new CreateDatakeyRequestBody()
                    .withDatakeyLength(String.valueOf(algorithm.getKeyLen() * 8))
                    .withKeyId(keyId)
